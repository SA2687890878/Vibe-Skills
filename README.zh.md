...
运行 `uninstall.ps1` 或 `uninstall.sh --host <host>` 是安装的对称路径，它默认 ledger-first 执行 owned-only 清理，最多只会删除 `.vibeskills/install-ledger.json`、`.vibeskills/host-closure.json`、或已经被 `adapters/*/closure.json` 记录的 Vibe 负载。完整契约见 [`docs/uninstall-governance.md`](./docs/uninstall-governance.md)，确保操作者清楚 Vibe 真正掌握的宿主文件。
