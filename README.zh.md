...
### 面向 Skills AI Agent 的即插即用工作 harness

#### 只要你的 AI 能加载 Skills，VibeSkills 就能为它补上一套受管工作流：入口简单、阶段清晰、自动编排专家 Skills、TDD/验证导向交付，并把关键上下文留给下一次会话。

&nbsp;
*VibeSkills 的核心价值不是“有 340+ 个 Skills”。真正重要的是它提供了一层 harness：什么时候澄清需求、什么时候规划、哪个阶段调用哪个专家 Skill、如何测试和验证、哪些信息要保存下来，都由这套受管流程统一组织。*
...
| 术语 | 通俗解释 |
|:---|:---|
| **Harness** | 包在 AI Agent 外层的受管工作流。它负责澄清、规划、执行、测试、验证，并保存有用上下文。 |
...
| **阶段编排** | 不同阶段调用不同 Skills：需求、规划、实现、评审、验证、清理，各司其职。 |
...
> ### 🎯 核心愿景
>
> VibeSkills 想把一个能加载 Skills 的 AI Agent，变成一套更有章法的工作系统。
>
> 它把受管 harness、专家 Skills、TDD 导向执行、验证规则和工作区记忆封装进一个即插即用的 Skills 包。用户不应该手动记住每个工具、手动催促每个阶段、手动盯住每个测试，也不应该在每次新会话里重新搭一遍上下文。
>
> **安装后调用 `vibe`，AI Agent 就获得一套结构化工作方式。**
> 目标不是给用户更多按钮，而是用更低的控制负担换来更稳定的交付纪律。
...
> **VibeSkills 关心的是更关键的问题：_"AI 应该如何组织工作、调用专家、测试结果，并把上下文留到下一次？"_**
...
| 层次 | 它替用户解决什么 |
|:---|:---|
| **简单入口** | 从 `vibe` 开始，用 `vibe-upgrade` 更新，不需要记一堆 workflow 命令。 |
| **受管 harness** | AI 按「澄清 → 规划 → 执行 → 测试 → 验证 → 保存上下文」推进，而不是直接乱跑。 |
| **阶段化 Skill 编排** | 专家 Skills 在真正有价值的阶段被拉起，而不是同时争抢控制权。 |
| **TDD 与验证纪律** | 任务结果必须落到测试、检查、证据或明确人工复核状态上，而不是一句“完成了”。 |
| **结构化上下文存储** | 需求、计划、决策、交接信息和证据存到可预期的位置，方便后续会话接上。 |
| **Skills 包可迁移** | 核心是即插即用的 Skills bundle，再通过适配层进入 Codex、Claude Code、Cursor、OpenCode、Windsurf、OpenClaw 等环境。 |
...
|:---|:---|
| 用户自己决定下一句提示词、下一个工具、下一步质量检查。 | harness 管理阶段边界，并在关键位置要求确认。 |
| Skills 只是一长串能力列表，AI 可能根本想不起来用。 | Skills 变成按阶段调度的专家工人。 |
| “完成”可能只是模型停止输出。 | 交付要绑定测试、检查、产物证据，或明确人工复核状态。 |
| 长任务跨会话后上下文丢失。 | 需求、计划、决策和证据被保存，方便继续。 |
| 每个宿主都要重新设计一套使用方式。 | 核心保持为 Skills 包，宿主适配围绕它展开。 |
...
VibeSkills 适合希望 AI Agent 更有结构、更少手动控制的人。
...
| ⚡ **重度使用 AI Agent 的人** | 需要一个 harness 来协调大量专家 Skills，支撑复杂工作。 |
| 🏢 **想规范 AI 工作流的团队** | 需要可复用的需求、计划、验证和交接产物。 |
| 🧩 **Skill 构建者与集成者** | 希望用 Skills 包作为核心封装，并适配到多种宿主环境。 |
| 😩 **厌倦手动调工具的人** | 希望系统自动判断哪个阶段该用哪个 Skill。 |

</div>

> _如果你只想要一个孤立小脚本，VibeSkills 可能偏重。但如果你希望 AI Agent 能跨阶段、跨会话承接真实工作，这就是让 Skills 规模化可用的那层基础设施。_
...
`vibe` 负责整个流程：什么时候澄清，什么时候规划，什么时候让专家 Skills 介入，什么时候跑测试或检查，以及什么时候允许声明交付完成。

<div align="center">

| 用户常见担心 | 实际怎么处理 |
|:---|:---|
| “Skills 太多了，我怎么选？” | 不需要你从完整列表里手动挑。harness 会按任务、阶段和约束路由。 |
...
- **按阶段调度专家**：需求、规划、实现、测试、评审、清理，可以各自调用不同 Skills。
